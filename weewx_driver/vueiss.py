...
class WindData(object):
    ...
    def add(self, data):
        ...
        direction = 360 if direction > 1024 or direction <= 0 else int(
            round(direction * 360.0 / 1024.0))
...
class StationParser(object):
    ...
    def parse(self, data, data_time):
        ...
        if self.packet_time != new_packet_time:
            ...
            _windSpeed = wind_info[0] if wind_info[0] is not None else None
            packet['windSpeed'] = round(
                _windSpeed, 2) if _windSpeed is not None else None
            _windSpeed = wind_info[1] if wind_info[1] is not None else None
            packet['windDir'] = round(
                _windSpeed, 0) if _windSpeed is not None else None

            _windGust = self.gust_data.get()
            packet['windGust'] = round(
                _windGust, 2) if _windGust is not None else None

            packet['rain'] = self.rain_data.get()

            _outTemp = self.temp_data.get()
            packet['outTemp'] = round(
                _outTemp, 1) if _outTemp is not None else None

            _outHumidity = self.humidy_data.get()
            packet['outHumidity'] = round(
                _outHumidity, 0) if _outHumidity is not None else None
    ...
    def __init__(self, config_dict):
        ...
        with weewx.manager.open_manager_with_config(self.config_dict, 'wx_binding') as dbmanager:
            with weedb.Transaction(dbmanager.connection) as cursor:
                ...
                DELTA = 600000
                logmsg("The time %d" % (self.the_time))
                if self.the_time - DELTA >= 0:
                    cursor.execute("SELECT dateTime,data FROM sensor WHERE dateTime>=%d AND dateTime<=%d ORDER BY dateTime ASC LIMIT 500" % (
                        self.the_time - DELTA, self.the_time))
    ...
    def genLoopPackets(self):
        ...
        while True:
            with weewx.manager.open_manager_with_config(self.config_dict, 'wx_binding') as dbmanager:
                with weedb.Transaction(dbmanager.connection) as cursor:
                    cursor.execute(
                        "SELECT dateTime,data FROM sensor WHERE dateTime>%d ORDER BY dateTime ASC LIMIT 5000" % (self.the_time))
